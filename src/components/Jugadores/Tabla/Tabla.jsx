...
export default function Tabla() {
    ...
    const { data, isLoading } = useSWR(`https://api.chaoschampionship.com/.netlify/functions/api/usuarios/limite=${rowsPerPage}&pagina=${page}`, fetcher, {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
            filteredUsers = filteredUsers.filter((user) =>
                Array.from(statusFilter).includes(user.linea_secundaria),
            );
        }
    ...
    return (
        ...
        >
            ...
            <TableBody emptyContent={"No users found"} items={data ?? []} loadingContent={<div>yo</div>} loadingState={loadingState}>
                {(item) => (
