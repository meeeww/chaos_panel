...
export default function ModalInscribirse({ inhouse ,blueLleno, redLleno, cambioDatos }) {
    ...
    const handleUpload = () => {
        toast.promise(() => new Promise((resolve, reject) => {
            if (side == 1) {
                inscribirseInhouse(inhouse.id_partido, usuario.info.id_usuario, side, blueLleno, cambioDatos.cambioDatos, resolve, reject)
            } else if (side == 2) {
                inscribirseInhouse(inhouse.id_partido, usuario.info.id_usuario, side, redLleno, cambioDatos.cambioDatos, resolve, reject)
            } else {
                reject()
